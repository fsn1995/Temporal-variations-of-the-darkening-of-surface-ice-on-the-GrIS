...
// export the mean albedo (visnirAlbedo) of the interpolated time series to EE asset
var meanAlbedo = regularCol.select(['visnirAlbedo']).mean().updateMask(greenlandmask);
...
// export the min albedo (visnirAlbedo) of the interpolated time series to EE asset
var minAlbedo = regularCol.select(['visnirAlbedo']).min().updateMask(greenlandmask);
Export.image.toAsset({
  image: minAlbedo,
...
// export the bare ice duration of the interpolated time series to EE asset
var bareIceDuration = regularCol.select(['visnirAlbedo']).map(function(img) {
  return img.updateMask(img.lt(0.565)).set('bareIceDuration', 1);
}).sum().updateMask(greenlandmask);
...
// export the dark ice duration of the interpolated time series to EE asset
var darkIceDuration = regularCol.select(['visnirAlbedo']).map(function(img) {
  return img.updateMask(img.lt(0.451)).set('darkIceDuration', 1);
}).sum().updateMask(greenlandmask);
