...
for i in range(len(df.aws)):
    ...
    aoi = ee.Geometry.Point([df.mean_gps_lon[i], df.mean_gps_lat[i]])
    Map.addLayer(aoi, {}, str(df.y[i]))
    date_start = str(df.y[i]) + '-' + str(df.m[i]) + '-' + str(1) 
    date_end = str(df.y[i]) + '-' + str(df.m[i]+1) + '-' + str(1) 
    # print(date_start)
    ...
    dfpoint['aws'] = df.aws[i]
    pointValueFile = "H:\AU\promiceaws\output\AWS_height_station_HSA.csv"
