...
for i in range(len(df.aws)):
    ...
    aoi = ee.Geometry.Point([df.lon[i], df.lat[i]])
    Map.addLayer(aoi, {}, str(df.year[i]))
    date_start = str(df.year[i]) + '-' + str(1) + '-' + str(1) 
    date_end = str(df.year[i]) + '-' + str(12) + '-' + str(31) 
    # print(date_start)
    ...
    pointValueFile = 'promice\hsa.csv'
